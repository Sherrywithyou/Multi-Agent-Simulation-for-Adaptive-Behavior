...
if __name__=="__main__":
    ...
    measure = 'trialScore'
    measureToLabel = {'trialScore': 'Accumulated Reward'; 'caughtTimes': 'Number of Catches'}
    ...
    dfToStat['label'] = 'trialScore'
    print(dfToStat)
    # dfOutputStat.to_csv(os.path.join(dirName, '..', 'results', 'individualRewardBaseResult_wolf14_sheep6_No6Folder_ToStat', 'IW_Accumulated Reward.csv'))
    ...
    print(dfToPlot)
    # dfOutputPlot.to_csv(os.path.join(dirName, '..', 'results', 'individualRewardBaseResult_wolf14_sheep6_No6Folder_ToPlot', 'IW_Accumulated Reward.csv'))
    
    # import ipdb; ipdb.set_trace()
    #sns.set_style("whitegrid")        # darkgrid(Default), whitegrid, dark, white, ticks
    f, ax = plt.subplots(figsize=(5, 5))
    # barplot: Default: np.mean
    g = sns.barplot(x='Stag Speed Multiplier', y='trialScore', hue = 'Set Size', data = dfMeanBySubAndCondtitionResetIndex, estimator=np.mean, ci=68, capsize=.05, errwidth=2, palette = 'Greys')#, color='Red', order = dfSelfAverageScore['targetColorIndexUnshuffled'], palette='Reds')
    ...
    plt.title('IW Model', fontsize=24)

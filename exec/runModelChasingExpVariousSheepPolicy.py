...
from env.multiAgentEnv import StayInBoundaryByReflectVelocity, TransitMultiAgentChasingForExpWithNoise, GetCollisionForce, \
    ...
    ObserveWithCaughtHistory, Observe, ReshapeActionVariousForce, ResetMultiAgentNewtonChasingVariousSheepWithCaughtHistory, \
    ...
    for numSheeps in manipulatedVariables['sheepNums']:
        for sheepConcern in manipulatedVariables['sheepConcern']:
            ...
            def loadPolicyOneCondition(numSheeps, sheepConcern):
                ...
                modelFolderName = 'newRewardIndividalAllSheep2block8Wepisode0.05dt1'
                ...
                # wolfFileName = "maddpg{}wolves{}sheep{}blocks{}episodes{}stepSheepSpeed{}WolfActCost0.0sensitive0.0biteReward0.0killPercent0.2_agent".format(numWolves, numSheeps, numBlocks, maxEpisode, maxTimeStep, modelSheepSpeed)
                ...
                sheepFileNameSep = "maddpg{}wolves1sheep{}blocks{}episodes{}stepSheepSpeed{}individ_agent3".format(numWolves, numBlocks, maxEpisode, maxTimeStep, modelSheepSpeed)
                ...
                # sheepFileNameAll = "maddpg{}wolves{}sheep{}blocks{}episodes{}stepSheepSpeed{}WolfActCost0.0sensitive0.0biteReward0.0killPercent0.2_agent".format(numWolves, numSheepToObserve, numBlocks, maxEpisode, maxTimeStep, modelSheepSpeed)

                # wolfModelPaths = [os.path.join(modelFolder, wolfFileName + str(i)) for i in range(numWolves)]
                wolfModelPaths = [os.path.join(modelFolder, wolfFileName + str(i) + str(evaluateEpisode) + 'eps') for i in range(numWolves)]
                # sheepModelPathsAll = [os.path.join(modelFolder, sheepFileNameAll + str(i)) for i in range(numWolves, numWolves + numSheepToObserve)]
                ...
                # old "self" concern sheep name rule (same folder + diff file name distinguisged by trainingId)
                ...
                # new "self" concern sheep name rule (diff folder Id + same file name)
                sheepModelPathsSep = [os.path.join(modelFolder, sheepFileNameSep + str(evaluateEpisode) + 'eps')for i in range(numSheeps)]

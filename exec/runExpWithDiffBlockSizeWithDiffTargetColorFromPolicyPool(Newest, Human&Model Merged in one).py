...
def main():
    ...
    numTrainedWolfFolder = 1
    wolfControllerType = 'human'
    physicsDT = 0.05 # 50ms
    ...
    for blockSize in manipulatedVariables['blockSize']:
        for targetColorIndex in manipulatedVariables['targetColorIndex']:
            for sheepMaxSpeed in manipulatedVariables['sheepMaxSpeed']:
                ...
                def loadPolicyPoolOneCondition(numSheeps, sheepConcern, blockSize):
                    # -----------observe--------
                    ...
                    if wolfControllerType == 'human':
                        ...
                        modelFoldersWolfList = [os.path.join(dirName, '..', 'model', modelFolderNameWolf + str(i+1)) for i in range(numTrainedWolfFolder)]
                        ...
                        wolfModelPaths = [os.path.join(modelFolderWolf, wolfFileName + str(i) + str(evaluateEpisodeWolf) + 'eps') for (modelFolderWolf, i) in it.product(modelFoldersWolfList, list(range(numWolves)))]

                        [restoreVariables(model, path) for model, path in zip(wolfModelsList, wolfModelPaths)]
                        wolfPolicyFun = lambda allAgentsStates, models, obs: [actOneStepOneModel(model, obs(allAgentsStates)) for model in wolfModelsList]
                ...
                sheepModelsList, sheepPolicy, wolfModelsList, wolfPolicy = loadPolicyPoolOneCondition(numSheeps, sheepConcern, blockSize)
                ...
                allSheepModels.update({(numSheeps, sheepConcern, blockSize): sheepModelsList})
                allWolfModels.update({(numSheeps, sheepConcern, blockSize): wolfModelsList})
    ...
    trial = NewtonChaseTrialAllCondtionPolicyPoolWithDiffBlocksWithDiffTargetColor(screen, killzone, sheepLife, targetColor, numWolves, numBlocks,
                 maxTrialStep, wolfActionUpdateInterval, sheepActionUpdateInterval, displayDT, sheepModelsList, wolfModelsList, 
